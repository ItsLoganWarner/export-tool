...
export default function ESCTab({
  ...
  // ─── parse `raw` to extract driveModes once ─────────
  ...
  // ─── build flat list of all editable fields ────────
  ...
  // ─── local state: which fields are checked + their values ───
  const [checked, setChecked] = useState({});
  const [values,  setValues]  = useState({});

  // init from pendingChanges & extracted
  useEffect(() => {
    const c = {}, v = {};
    fields.forEach(f => {
  ...
  // split top-level vs per-mode
  const topFields = fields.filter(f => f.path.length === 2);
  const modeFields = fields.filter(f => f.path.length > 2);

  // the available modes (multi-select result)
  const availableModes = values.enabledModes || [];

  // selected mode state
  const [selMode, setSelMode] = useState('');
  useEffect(() => {
    if (!availableModes.includes(selMode)) {
      // prefer defaultMode if available
      const def = values.defaultMode;
      setSelMode(availableModes.includes(def) ? def : availableModes[0] || '');
    }
  }, [availableModes, values.defaultMode]);
  ...
  return (
    <div className="card esc-card">
      ...
      {/* Settings for only the selected mode */}
      {selMode && (
